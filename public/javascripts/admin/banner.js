...
const bannerPreviewImg = document.getElementById('image');


// Button
const btnWriteMou = document.getElementById('btn-banner-write');
const btnUpdateMou = document.getElementById('btn-banner-update');
const btnDeleteMou = document.getElementById('btn-banner-delete');


// Variable
let bannerDeleteList = [];
let bannerNotClick = true;
...
if(bannerTable) {
    ...
    $(document).ready(function() {
        const t = $('#banner-table').DataTable();
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                // banner list 값을 추출했으면
                if (typeof(result) == 'object') {
                    const faqList = result;

                    // grid에 banner list 추가
        ...
        $('#banner-table tbody').on( 'click', 'tr', function () {
            // 링크 이동이 아닐 경우만
            if (bannerNotClick) {
                ...
                if (cl.includes('selected')) {
                    ...
                    const index = bannerDeleteList.indexOf(num);

                    if (index > -1) {
                        bannerDeleteList.splice(index, 1);
                    } else {}
                ...
                // 선택되어 있지 않다면 선택
                    ...
                    bannerDeleteList.push( t.row( this ).data()[0].split('preventMouActive(')[1].split(')')[0] );
...
function preventMouActive(id) {
    ...
    location.href = `/admin/banner/update?id=${id}`
}
...
if (bannerPreviewImg) {
    bannerPreviewImg.addEventListener('change', (e) => {
...
if (btnWriteMou) {
    btnWriteMou.addEventListener('click', () => {
        ...
        if (name.value == '') {
            ...
            const formData = new FormData(document.getElementById('banner-write'));

            downButton();

            $.ajax({
                url: `/admin/banner/write`,
                data: formData,
                type: "post",
...
if (btnUpdateMou) {
    btnUpdateMou.addEventListener('click', () => {
        ...
        if (name.value == '') {
            ...
            // banner 기관 정보를 수정한다.
            const formData = new FormData(document.getElementById('banner-update'));
...
if (btnDeleteMou) {
    btnDeleteMou.addEventListener('click', () => {
        if (bannerDeleteList.length != 0) {
            const isDelete = confirm(`${bannerDeleteList.length}개 항목을 삭제하시겠습니까?`);
            ...
            if (isDelete) {
                ...
                $.ajax({
                    ...
                    data: { bannerDeleteList },
                    type: "delete",
                    success: function (result) {

                        // banner 기관이 성공적으로 삭제되었으면
                        if (result) {
                            alert('삭제되었습니다.');
                            location.reload();
                        }
